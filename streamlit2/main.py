...
def open_pdf():
    st.set_page_config(page_title="My PDF", page_icon=":pdf:", layout="wide")
    pdf_path = "Ulugbek_Zayniev_CV.pdf"
    st.markdown("<a href='"+pdf_path+"' target='_blank'>Open PDF</a>", unsafe_allow_html=True)


   
if st.button("Result", key=time.time()):
    open_pdf()
